...
- Keep each subtask internally serial in the order Implementer -> Tester -> Documenter -> Verifier, inserting the specialist Security stage between Documenter and Verifier when the plan marks the subtask as requiring security review.
- Track dependency status, current stage, security-required flag, worktree paths, artifact directory, remediation counts, merge status, and the full planned-subtask completion set.
