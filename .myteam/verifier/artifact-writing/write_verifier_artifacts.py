...
def validate_verdict(verdict: str, counts: dict[str, int]) -> None:
    if verdict not in VERDICTS:
        raise SystemExit(f"Invalid verdict: {verdict}")
    if verdict == "PASS" and counts["BLOCKING"] > 0:
        raise SystemExit("PASS verdict is invalid when blocking findings are present.")
...
def render_report(payload: dict[str, object], findings: list[dict[str, str]], artifact_paths: list[str]) -> str:
    lines: list[str] = ["Verifier Report", ""]
    ...
    lines.append("")
    lines.extend(render_list_block("Convention files considered:", as_list(payload.get("convention_files_considered"))))
    lines.append("")
    ...
    lines.append("")
    lines.extend(render_list_block("Documentation accuracy assessment:", as_list(payload.get("documentation_accuracy_summary"))))
    lines.append("")
    lines.extend(render_list_block("Artifacts written:", artifact_paths))
    lines.append("")
    lines.extend(render_list_block("Verdict:", [str(payload["verdict"])]))
    return "\n".join(lines) + "\n"


def main() -> int:
    parser = argparse.ArgumentParser(description="Write verifier artifacts from structured JSON input.")
    ...
    required = [
        ...
        "plan_reference",
        "convention_files_considered",
        "test_sufficiency_summary",
        "documentation_accuracy_summary",
    ]
    ...
    validate_verdict(str(payload["verdict"]), counts)
    ...
    report_path = artifact_dir / "verifier_report.md"
    result_path = artifact_dir / "verifier_result.json"
    ...
    result_payload = {
        ...
        "documentation_accuracy_summary": as_list(payload.get("documentation_accuracy_summary")),
