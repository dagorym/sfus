...
### Summary
Updated the CI/CD docs to match the remediated container-runner behavior: local container runs still no-op with a warning when a compose file has no services, and custom compose files are documented as supporting inline-map plus non-fixed-indentation `services` layouts for local runs.
