...
cleanup() {
  DOCKER_BUILDKIT="${docker_buildkit}" docker compose --env-file "${root_env_file}" -p "${project_name}" -f "${compose_file}" --profile fullstack down -v --remove-orphans >/dev/null 2>&1 || true
...
DOCKER_BUILDKIT="${docker_buildkit}" docker compose --env-file "${root_env_file}" -p "${project_name}" -f "${compose_file}" --profile fullstack up -d --build

echo "==> [startup] Waiting for homepage and API liveness"
wait_for_http_ok "http://127.0.0.1:3000/" "homepage"
wait_for_http_ok "http://127.0.0.1:3001/api/health/live" "API liveness"

echo "==> [migration] Running explicit migration command"
DOCKER_BUILDKIT="${docker_buildkit}" docker compose --env-file "${root_env_file}" -p "${project_name}" -f "${compose_file}" --profile fullstack run --rm api node dist/index.js migration:run

echo "==> [validation] Verifying homepage contract"
homepage_response="$(curl -fsS http://127.0.0.1:3000/)"
...
api_live_response="$(curl -fsS http://127.0.0.1:3001/api/health/live)"
...
wait_for_http_ok "http://127.0.0.1:3001/api/health/ready" "API readiness"
api_ready_response="$(curl -fsS http://127.0.0.1:3001/api/health/ready)"
