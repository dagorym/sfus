...
- `bash cicd/tests/run-validations.sh`: validates that `apps/api/.env.example` uses the hybrid-dev default `DB_HOST=127.0.0.1` (Compose overrides to `mysql` for container runs) and that `compose.dev.yml` includes the `DB_HOST: mysql` service-level override; also validates that the API Docker image stays running with the required env vars including `MEDIA_STORAGE_PATH`
- `bash cicd/tests/run-validations.sh`: validates default-run success, warning-only success, no GitHub Actions `::warning::...` token on stdout or stderr when `GITHUB_ACTIONS` is unset, and in Actions mode verifies that the human warning text remains visible while the `::warning::...` workflow command is emitted on the parsed output channel, plus missing-config failure and strict missing-command failure
