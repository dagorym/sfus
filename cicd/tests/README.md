...
- `bash cicd/tests/build-images.sh`: validates configured image parsing and build invocation, the `validation` alias, empty-matrix warning-only success, publish/deploy gate behavior, invalid-operation failure, and docker-unavailable failure behavior
...
- `bash cicd/tests/run-validations.sh`: validates that image targets live under `cicd/config/image-matrix.yml`, that publish/deploy stay disabled by default there, and that `.github/workflows/cd.yml` remains a manual-dispatch shim over `cicd/scripts/build-images.sh` plus `cicd/config/image-matrix.yml`
