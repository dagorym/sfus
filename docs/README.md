...
- MFA is implemented with TOTP plus recovery codes: authenticated users start enrollment at `POST /api/auth/mfa/enroll`, confirm at `POST /api/auth/mfa/enroll/verify`, and receive one-time recovery codes only after successful verification. Recovery-code regeneration (`POST /api/auth/mfa/recovery/regenerate`) and disable (`POST /api/auth/mfa/disable`) both require authenticated MFA proof.
