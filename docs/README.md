...
- The authenticated shell includes `/app`, `/profile`, `/settings`, and `/onboarding/username`; authenticated routes redirect unauthenticated users to `/login` and redirect `user.onboardingRequired` sessions into username completion before normal authenticated use.
