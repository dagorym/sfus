...
## Dependency Ordering
- No subtask is a functional prerequisite of another; all five change disjoint behavior.
- Shared-file serialization: `subtask-3`, `subtask-4`, and `subtask-5` each edit `docs/README.md`. To avoid merge contention they should be executed **serially in the order 3 → 4 → 5** (or have their `docs/README.md` edits coordinated by the Coordinator).

Parallelization notes:
- Web group `{subtask-1, subtask-2}` touches disjoint files (`register`/`login` vs `page.tsx`/`page.module.css`/new feed component) and is safely parallelizable with each other.
- The web group `{subtask-1, subtask-2}` and the API group `{subtask-3, subtask-4, subtask-5}` are independent and may run concurrently.
- Within the API group, prefer serial execution because of the shared `docs/README.md` edits; the underlying code changes are otherwise independent.
